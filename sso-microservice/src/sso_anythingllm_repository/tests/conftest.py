...
async def test_db(event_loop) -> AsyncGenerator[AsyncPostgresConf]:
    """
...
async def sql_connector(test_db: AsyncPostgresConf) -> AsyncGenerator[AsyncSqlConnector]:
    """
    ...
    di[AsyncConnector] = connector  # open pool so it's ready for the tests
    yield connector
...
async def property_type(db_session: AsyncSession) -> ProviderPropertyType:
    ...
    unique_type = f"text_{uuid.uuid4().hex[:8]}"
    property_type = ProviderPropertyType(type=unique_type, description="Test Property Type")
...
async def provider_instance(
    ...
    provider_instance = ProviderInstance(
        ...
        description="Test provider instance for testing",
        overridable=True,
    )
